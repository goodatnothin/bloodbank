...
int main(){
	...
	unordered_map<string,vector<Donor>> donor_list;
	while(num--){
		...
		auto itr5=donor_list.find(blood_group);
		if(itr5 == donor_list.end()){	
			vector<Donor> donor_dt1;
			donor_dt1.push_back(Donor(name,rollnum,phonenum,email,addhar_no,blood_group,dept,num_of_blood,lst_dt_of_bd));
			donor_list.insert({blood_group,donor_dt1});
		}
		else{
			itr5->second.push_back(Donor(name,rollnum,phonenum,email,addhar_no,blood_group,dept,num_of_blood,lst_dt_of_bd));
		}
	}			
	infile.close();
	ofstream fout;
	...
	while(1)
	{
		...
		unordered_map<string,vector<Donor>>::iterator itr6=donor_list.begin();
		unordered_map<string,vector<Donor>>::iterator itr7=donor_list.end();
		string name,phonenum,email,addhar_no,blood_group,dept,lst_dt_of_bd;
						long int rollnum;
						int num_of_blood;
		switch(ch)
			...
			case 5:
				...
				{	
					...
					while(itr6 != itr7)
					{
						i=0;
						cout<<itr6->first;
						auto ptr2=itr6->second.begin();
						auto ptr3=itr7->second.end();
						while(ptr2 != ptr3)
						{
							i++;
							ptr2->print1();
						...
						cout << "\nTotal count for "<<itr6->first<< "is : "<< i  ; 
						itr6++;
						cout<<endl;
					}
				}
